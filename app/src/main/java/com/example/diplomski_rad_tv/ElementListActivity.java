...
public class ElementListActivity extends Activity {
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if (keyCode >= 19 && keyCode <= 22) {
            ...
            // Pressed down
                ...
                if (BasicGridNavigation.isUpperButtons(oldFocusedViewId)) {
                    ...
                    if (this.grid == BasicGridNavigation.six && this.overallIndex + 3 < this.elementsToShow.size()) {
                        this.overallIndex += 3;
                        this.currentIndex += 3;
            ...
            // Pressed right
                ...
                if (BasicGridNavigation.isUpperButtons(oldFocusedViewId) || BasicGridNavigation.isLowerButtons(oldFocusedViewId)) {
                    ...
                    if (this.overallIndex + 1 < elementsToShow.size()) {
                        this.overallIndex++;
                        this.currentIndex = this.overallIndex % gridType;
                    ...
                    if (BasicGridNavigation.isRightColumn(oldFocusedViewId)) {
                        if (this.overallIndex + 4 < elementsToShow.size()) {
                            this.overallIndex += 4;
                            this.currentPage = this.overallIndex / gridType;
                            this.currentIndex = this.overallIndex % gridType;
                            setNewContentView();
        ...
        // Enter button
            ...
            if (oldFocusedViewId == R.id.languageButton) {
                ...
                this.navigateToDescriptionActivity();
            }
        }
    ...
    void setNewContentView() {
        ...
        this.currentPage = this.overallIndex / gridType;
        this.currentIndex = this.overallIndex % gridType;
        this.totalPages = (this.elementsToShow.size() - 1) / gridType + 1;
    ...
    void setupMain() {
        ...
        if (this.loadingInProgress) {
            this.setTitleText(View.VISIBLE, R.string.element_name_en, R.color.text_color_dark_mode);
            this.setCenterText(View.INVISIBLE, R.string.empty_string, R.color.transparent);
        } else if (this.elementsToShow.size() == 0 /*&& !this.loadingInProgress*/) {
            ...
            switch (this.language) {
                case united_kingdom:
                    if (this.theme == Theme.dark) {
                        this.setTitleText(View.VISIBLE, R.string.element_name_en, R.color.text_color_dark_mode);
                        this.setCenterText(View.VISIBLE, R.string.no_elements_en, R.color.text_color_dark_mode);
                    } else if (this.theme == Theme.light) {
                        this.setTitleText(View.VISIBLE, R.string.element_name_en, R.color.text_color_light_mode);
                        this.setCenterText(View.VISIBLE, R.string.no_elements_en, R.color.text_color_light_mode);
                    }
                    break;
                case germany:
                    if (this.theme == Theme.dark) {
                        this.setTitleText(View.VISIBLE, R.string.element_name_de, R.color.text_color_dark_mode);
                        this.setCenterText(View.VISIBLE, R.string.no_elements_de, R.color.text_color_dark_mode);
                    } else if (this.theme == Theme.light) {
                        this.setTitleText(View.VISIBLE, R.string.element_name_de, R.color.text_color_light_mode);
                        this.setCenterText(View.VISIBLE, R.string.no_elements_de, R.color.text_color_light_mode);
                    }
                    break;
                case croatia:
                    if (this.theme == Theme.dark) {
                        this.setTitleText(View.VISIBLE, R.string.element_name_hr, R.color.text_color_dark_mode);
                        this.setCenterText(View.VISIBLE, R.string.no_elements_hr, R.color.text_color_dark_mode);
                    } else if (this.theme == Theme.light) {
                        this.setTitleText(View.VISIBLE, R.string.element_name_hr, R.color.text_color_light_mode);
                        this.setCenterText(View.VISIBLE, R.string.no_elements_hr, R.color.text_color_light_mode);
                    }
                    break;
            }
        ...
        } else if (this.overallIndex < this.elementsToShow.size()) {
            this.hideProgressBar();

            this.setCenterText(View.INVISIBLE, R.string.empty_string, R.color.transparent);
            if (this.theme == Theme.dark) this.setTitleText(View.VISIBLE, this.elements[this.elementsToShow.get(this.overallIndex)].title, R.color.text_color_dark_mode);
            else if (this.theme == Theme.light) this.setTitleText(View.VISIBLE, this.elements[this.elementsToShow.get(this.overallIndex)].title, R.color.text_color_light_mode);
    ...
    void setupPaginationButton() {
        ...
        if (this.elementsToShow.size() != 0) paginationCurrentPage.setText(Integer.toString(this.currentPage + 1));
    ...
    void setupElementsToShow() {
        ...
        for (int i = 0; i < elements.length; ++i) {
            if ((elements[i].title.toLowerCase()).contains(this.searchbarText.toLowerCase())) this.elementsToShow.add(i);
        }
    }
    ...
    void setGridButton(View[] views, int drawableId, int viewIndex) {
        ...
        int visibility = viewIndex >= this.elementsToShow.size() ? View.INVISIBLE: View.VISIBLE;
        String titleText = viewIndex < this.elementsToShow.size() ? this.elements[this.elementsToShow.get(viewIndex)].title : "";
    ...
    void setGridButtonImage(ImageButton imageButton, int viewIndex) {
        if (viewIndex < this.elementsToShow.size() && this.elements[this.elementsToShow.get(viewIndex)].background != null && !this.elements[this.elementsToShow.get(viewIndex)].background.isEmpty()) {
            try {
                Picasso.get()
                        .load(this.elements[this.elementsToShow.get(viewIndex)].background)
                        .fit()
                        .into(imageButton);
            } catch (Exception e) {
                e.printStackTrace();
    ...
    void setGridButtonImage(ImageView imageButton) {
        if (this.elements[this.elementsToShow.get(this.overallIndex)].background != null && !this.elements[this.elementsToShow.get(this.overallIndex)].background.isEmpty()) {
            try {
                Picasso.get()
                        .load(this.elements[this.elementsToShow.get(this.overallIndex)].background)
                        .fit()
                        .into(imageButton);
            } catch (Exception e) {
                e.printStackTrace();
    ...
    void navigateToDescriptionActivity() {
        String elementId = this.elements[this.elementsToShow.get(this.overallIndex)].id;
        ...
        Toast.makeText(ElementListActivity.this, "Navigate to next page", Toast.LENGTH_SHORT).show();

        Gson gson = new Gson();
        String sendValues = gson.toJson(this.elements[this.elementsToShow.get(this.overallIndex)]);

        Bundle bundle = new Bundle();
        bundle.putString("element", sendValues);
